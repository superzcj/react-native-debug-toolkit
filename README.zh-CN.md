...
Remote JS Bundle：先在电脑启动 Metro，在 `DevConnect` 输入电脑 IP 和 Metro 端口，然后点 `Use Metro Bundle`。DevConnect 会写入 React Native 的 packager host（iOS 上即 `RCTBundleURLProvider.jsLocation`）并从新的 Metro 热重载。

> **仅 Debug 包可用。** 切换 Metro host 只在 Debug 构建生效。Release 包加载内置的 `main.jsbundle`，且 RN 会把 packager 相关机制编译掉（`RCT_DEV=0`），因此 Release 下控件会被禁用，面板显示 `release: disabled` 徽标。如果需要让优化过的包连 Metro，请构建 development/dev-client 包（Debug 配置），而不是生产 Release 包。
