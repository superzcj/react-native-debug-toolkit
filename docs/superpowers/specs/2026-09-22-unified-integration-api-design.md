# 统一 Feature 配置与 App 接入设计

- 日期：2026-09-22
- 状态：方向已确认；完整设计稿待审阅，尚未实现
- 范围：SDK 公开 API、初始化与生命周期、全部现有对外功能、接入文档、Demo 与消费者验收
- 发布方式：破坏性变更，以新的主版本发布；不保留旧 API、字段别名和兼容初始化路径

## 1. 用户目标与已确认约束

1. 常见 App 安装依赖并完成原生构建后，只需一行根组件包装即可使用。
2. 每个 feature 使用对象配置，统一启停、默认值、校验和生命周期规则。
3. 默认配置可用，业务增强逐步添加；普通使用不需要理解内部 Feature、Controller 或存储适配器。
...
选择一个根组件包装入口，配合统一的对象配置。舍弃纯副作用 `import '.../auto'` 作为通用接入方式，避免全局修改 AppRegistry 和依赖宿主注册时机；不同时维护 JSX 包装、初始化函数与多个推荐入口。
...
```ts
type FeatureConfig<T extends object> =
  | { enabled: false }
  | (T & { enabled?: boolean });
```

业务必填参数属于 `T`。例如 `accounts: { enabled: false }` 不必提供账号；启用账号功能则必须提供完整数据和回调。动态 boolean 开关也不能绕过业务必填参数检查。

统一规则：

1. 省略 feature：采用下表明确规定的默认行为。
2. 提供对象且省略 `enabled`：请求启用该 feature，并对可选参数应用默认值。
3. `enabled: false`：不挂载面板、不安装采集/订阅、不执行该功能的后台任务。不是只隐藏 Tab。
4. 不接受隐式深合并。每个 feature 的字段单独应用默认值；数组整体使用调用方提供的值；`undefined` 视为省略，`null` 只有字段明确允许时有效。
5. 业务必填项缺失、未知字段、重复 ID 和非法数值均产生带字段路径的错误。
6. 配置初始化后固定。业务动态数据通过订阅源更新，不在 render 中重建配置，不提供任意运行时 `setConfig`。
7. 配置的启用意图与实际能力分开：状态可为 `disabled`、`initializing`、`ready`、`unavailable`、`error`，必须带原因。无数据不等于未启用。
...
下表的开关默认值均以全局已经允许启用为前提。日志容量统一使用 `maxLogs`，默认 200 条内存记录；正整数参数必须校验，不静默截断非法值。

| Feature | 省略时的行为 | 启用对象的参数与默认值 | 业务依赖 |
| --- | --- | --- | --- |
...
| `native` | 请求开启，按能力判断 | `maxLogs: 200`；`minLevel` 省略表示不过滤；`includeTags: []`、`excludeTags: []`；`pollIntervalMs: 500`，最小 100 | Toolkit 原生模块；不可用时不保留空轮询 |
...
| `connect` | 请求开启 | 可选 `appId`、`endpoint`，均自动推导或发现 | 原生 App 信息、可达 Hub；缺失时不阻断本地面板 |
| `clipboard` | 开启文本工具 | 仅通用 `enabled` | 系统剪贴板是可选能力；电脑传递需要 Console/Hub 通道 |
| `history` | 开启 | `maxSessions: 5`，包括当前会话 | 本地持久化；范围见第 5.5 节 |
| `environment` | 关闭 | 必填 `items`、`defaultId`；可选 `onChange` | 业务服务 URL 清单 |
| `accounts` | 关闭 | 必填账号数据、`onSwitch`；高级选项见第 5.3 节 | 业务执行实际认证 |
| `tabs` | 关闭 | 必填非空 `items` | React 组件；可选快照数据源 |

`state`、`navigation`、`track` 没有数据时，面板说明其接入方式。零配置不意味着自动发现任意 store、路由库和埋点系统。
...
下列摘录展开影响必填关系和类型推断的部分；账号的其他可选回调按第 5.3 节加入 `AccountsOptions`，不改变这里的互斥关系。`ComponentType` 使用 React 类型，`DebugSource` 定义见第 5.1 节。
...
type AccountsData<A extends DebugAccount> =
  | (AccountsSnapshot<A> & { source?: never })
...
- `{ accounts: {} } satisfies DebugToolkitConfig` 报缺少业务数据和 onSwitch。
...
- 裸 boolean、数组/ref，以及缺少必填业务字段的动态 enabled boolean 均报错。
...
单服务只填一个 `api` key，多服务增加命名 key，不再引入另一种配置形状。每个环境使用相同服务 key，`defaultId` 必须命中唯一 ID；URL 必须是绝对 HTTP(S) URL。同一环境内歧义的相同前缀配置被拒绝。
...
高级业务编排通过 `debug.accounts.switchTo(id)`、`suspend()`、`resume()`、`waitForIdle()` 操作当前功能，不再获得内部 Controller。`switchTo` 返回 success/error/superseded/disabled/busy 的结构化结果；挂起禁止新切换并取消当前切换，等待空闲覆盖在途回调和补偿完成。业务 Promise 不结束时 waitForIdle 也不能假称空闲，面板保持 busy/suspended 并说明原因。最近账号加载纳入 `debug.ready()`，不再公开独立 `waitForStorage`。
...
未通过全局启用门禁前不安装采集。`debug.ready()` 的完成只表示允许启用的本地功能已完成初始化或给出失败结果，不等待无限 Hub 重试，不保证此前的 render、模块 import 或网络事件被追溯捕获。完整启动期 bootstrap 作为独立后续需求，不给主入口增加第二次初始化步骤。

全局配置错误使 Toolkit 停用并通过原始开发日志和 ready 结果给出字段路径；某个 feature 的配置错误使该 feature 停用，其他独立功能继续。面板可用时也显示错误，状态页不能把部分失败显示为全部就绪。回调/订阅异常应隔离并释放相关资源，不让调试工具打断业务 App。
...
1. README：最短安装步骤、完整最小 App 示例、默认能力和验证方法。
2. `docs/integration.md` 与中文对应页：单页自包含指南，包含项目识别、受支持入口的修改前后示例、原生重编译、Hub 连接、Release 边界及排障。
3. `docs/configuration.md` 与中文对应页：全部 feature 的字段类型、必填项、默认值、禁用语义、实际采集范围和高级数据源例子。

AI 阅读路径必须能回答：装什么、改哪个入口、替换什么代码、怎么配置、何时需要重新构建、如何验证。最小示例不引用仓库私有 helper、不留未定义变量，也不要求读内部源码才能完成。
...
保留完整购物场景作为 Showcase，使用新的公开 API 展示状态、导航、埋点、环境、账号和自定义 Tab。另提供最小入口，只包装根组件，包含产生 Console 和真实请求的按钮，不手动初始化，不预填 appId/Hub endpoint，不接业务适配器。

消费者 fixture 必须在父仓库以外的干净临时目录中安装 `npm pack` 产物。它可以复用最小 Demo 的业务组件，但不能保留父目录源码 alias、Jest/TS paths、iOS path pod 或 Android project include，也不能依靠父 node_modules 补齐依赖。先构建包，再验证 tarball 内的 JS、类型、原生文件和文档入口。

AI 验收只给未接入的 fixture、公开文档入口和任务要求；不给私有源码解释、不安装接入 Skill、不提供预先接好的根组件。记录 AI 的补丁、执行命令、阻塞项和当前会话证据；缺少设备或构建环境时报告未完成，不计为通过。

| 验收项 | 通过标准 |
| --- | --- |
| 全部 feature 的对象协议 | 每项省略/合法对象/关闭/错误字段都覆盖；裸 boolean/数组/ref 和旧字段被拒绝 |
| 零配置接入 | 无业务配置即可打开面板、记录 Console/Network，Hub 可达时识别正确 App |
...
| 环境 | 单/多服务、前缀边界、恢复默认、持久化恢复、回调失败；Network/Environment 分别关闭和反复初始化不误删对方资源；仅环境开启时不记录请求 |
| 账号 | 静态/动态数据、作用域与目标对象变化、展示快照更新不取消自身、取消订阅、忽略 signal 的过期回调、取消/回滚/重复点击 |
| 自定义 Tab | 普通组件、source 更新、Context 桥接、ID 冲突、订阅清理 |
...
1. 建立统一类型、默认值、校验与 feature 注册定义；明确新公共出口与旧出口移除清单。
2. 实现单宿主运行时、生命周期和共享资源所有权，再接入 HOC。
3. 逐项迁移全部功能，完成业务适配器与手动动作接口，不以“配置能编译”代替行为验收。
4. 更新最小 Demo、完整 Showcase 和干净消费者 fixture。
